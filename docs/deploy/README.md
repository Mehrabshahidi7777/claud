...
> ⚠️ **پیش از اولین ارسال واقعی:** نام پارامترهای `SendWithPattern` بین نسخه‌های پنل
> آموت فرق دارد و این بخش با پنل واقعی آزموده نشده. اگر پیامک نرفت، خطای پنل در
> `storage/logs` هست؛ آن را برای برنامه‌نویس بفرستید. اصلاحش فقط در
> `app/Sms/Drivers/AmootSmsDriver.php` است.
>
