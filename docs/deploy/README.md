...
تبدیل یک پاراگراف به کار ۲۰ تا ۶۰ ثانیه طول می‌کشد.
...
> ⏳ **از همین امروز شروع کنید، چون چند روز طول می‌کشد:**
> - **پترن‌های آموت:** متن پترن‌ها باید در پنل تأیید شود. فقط پترن `otp` (کد ورود)
>   برای شروع لازم است.
>
> تا آماده شوند سایت با پیامک «لاگ» کامل کار می‌کند.
>
> **درگاه سامان فعلاً لازم نیست:** پیگیر برای همه رایگان است و هزینه‌اش را
> اسپانسرها می‌دهند (`BILLING_ENABLED=false`). اگر روزی خواستید اشتراک بفروشید،
> بخش ۱۱ را ببینید.
...
`SMS_DRIVER=log` و `AI_PROVIDER=null` بمانند تا بخش‌های ۱۰ و ۱۲.

این‌ها از `.env.example` درست می‌آیند و **دست نزنید**:

```ini
APP_LOCALE=fa                                   # پیام‌های خطا فارسی
...
> کارها و شماره‌های ساختگی (`0912111000…`) فقط روی کامپیوتر خودتان هستند و چیزی
...
> **پوشه‌ی `public_html` لازم نیست.** آن مخصوص هاست اشتراکی (سی‌پنل) است. روی سرور
> مجازی خودتان تعیین می‌کنید سایت از کجا خوانده شود: فایل `nginx.conf` پروژه
> ریشه‌ی سایت (document root) را روی `/var/www/peygir/public` می‌گذارد. هیچ فایلی را
> جابه‌جا نکنید؛ پروژه همان‌جا که `git clone` شده می‌ماند و بقیه‌ی پوشه‌ها (`.env`،
> `app`، `storage`) از بیرون دیده نمی‌شوند، که برای امنیت هم بهتر است.
...
> **یادتان باشد — نسخه‌ی اندروید:** حالا که دامنه با `https` بالا آمد، وقت ساختن
> فایل APK برای کافه‌بازار و مایکت است. به Claude بگویید «دامنه بالاست، APK را
> بساز» و آدرس دامنه را بدهید. نصب مستقیم از مرورگر روی گوشی هم از همین لحظه کار می‌کند.
...
- **کاربرها** خودشان با شماره‌ی خودشان وارد می‌شوند، اسم و شرکتشان را می‌دهند و
  رایگان شروع می‌کنند. کسی که مالک یا مدیر در بخش «اعضا» اضافه کند، با همان شماره
  وارد می‌شود و مستقیم به فضای کاری همان شرکت می‌رود.
- **اسپانسرها:** در پنل مدیریت کل، تب **«اسپانسرها»** ← «اسپانسر تازه». نام، لوگو
  (PNG، JPG یا WebP تا ۱ مگابایت)، یک توضیح کوتاه و آدرس سایت. همان لحظه در صفحه‌ی
  ورود، پایین داشبورد همه و صفحه‌ی «اسپانسرهای پیگیر» دیده می‌شود. تعداد کلیک روی
  هر اسپانسر هم همان‌جا شمرده می‌شود؛ برای تمدید قرارداد با اسپانسر به دردتان می‌خورد.
...
**شماره‌ی خط:** دقیقاً همانی که آموت برای ارسال با خط اختصاصی گفته، بدون فاصله و
بدون `+`. ممکن است فقط `98` باشد؛ اگر آموت همین را گفته، همان `98` درست است. اگر پنل برای ارسال پترنی
...
`https://portal.amootsms.com/rest` می‌ماند (بدون اسم متد در آخرش؛ اگر اشتباهی `SendWithPatternOWN` را هم آخرش بگذارید، برنامه خودش حذفش می‌کند).
...
> **فعلاً لازم نیست.** پیگیر رایگان است (`BILLING_ENABLED=false`) و صفحه‌های
> اشتراک و پرداخت اصلاً نمایش داده نمی‌شوند. این بخش فقط برای روزی است که
> بخواهید دوباره اشتراک بفروشید؛ آن روز در `.env` بگذارید `BILLING_ENABLED=true`.
...
| **بانک سامان** (فعلاً لازم نیست) | شماره‌ی ترمینال | `SEP_TERMINAL_ID` |
...
`BILLING_ENABLED=false` یعنی پیگیر برای همه رایگان است و با اسپانسر اداره می‌شود:
نه دوره‌ی آزمایشی، نه فاکتور، نه پیامک تمدید. `SUBSCRIPTION_TRIAL_DAYS` و
`REFERRAL_BONUS_DAYS` فقط وقتی اثر دارند که روزی آن را `true` کنید.
سقف کد ورود برای شماره‌های تازه هم `OTP_NEW_NUMBERS_HOURLY_LIMIT=100` است (در ساعت)؛
جلوی کسی را می‌گیرد که بخواهد با درخواست کد برای هزاران شماره اعتبار پیامک را خالی کند.
مشتری‌های فعلی شامل این سقف نمی‌شوند. اگر پر شود، پنل مدیریت کل هشدار قرمز می‌دهد.
همه‌ی این‌ها از `.env.example` خودشان می‌آیند؛ فقط اگر خواستید عددشان را عوض کنید سراغشان بروید.
