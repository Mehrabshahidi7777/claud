...
@php
    ...
    $groups = [
        ...
        'سازمان' => [
            ...
            // Free, anyone may bring a friend. Paid, the reward is days on the
            // subscription, which is the payer's business.
...
<nav class="space-y-5 text-sm">
    ...
    @if (\App\Models\Sponsor::showcase()->isNotEmpty())
        <div>
            <a href="{{ route('sponsors.index') }}" @if ($current === 'sponsors.index') aria-current="page" @endif
               class="flex items-center gap-3 rounded-lg px-3 py-2 transition
                      {{ $current === 'sponsors.index'
                          ? 'bg-brand-50 font-medium text-brand-700'
                          : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' }}">
                {{-- Two banknotes with a heart: the money that keeps پیگیر free, and
                     the thanks for it. Drawn to match the outline icons. --}}
                <svg class="size-5 shrink-0 {{ $current === 'sponsors.index' ? 'text-brand-600' : 'text-emerald-600' }}"
                     fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 6h12.75A1.5 1.5 0 0 1 21 7.5v7.5M4.5 9h12A1.5 1.5 0 0 1 18 10.5v7.5a1.5 1.5 0 0 1-1.5 1.5h-12A1.5 1.5 0 0 1 3 18v-7.5A1.5 1.5 0 0 1 4.5 9ZM5.4 11.4h.01M15.6 16.95h.01"/>
                    <path fill="currentColor" fill-opacity=".25" stroke-linecap="round" stroke-linejoin="round" d="M10.5 17.1l-2.25-2.2a1.4 1.4 0 0 1 2.25-1.65 1.4 1.4 0 0 1 2.25 1.65Z"/>
                </svg>
                اسپانسرهای پیگیر
            </a>
        </div>
